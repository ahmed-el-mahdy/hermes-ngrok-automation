...
Run `bash configure-telegram-media.sh` to enable the persistent media profile. It builds `Dockerfile.hermes-agent` with Hermes' pinned `faster-whisper` dependencies and guarded STT/media patches, configures forced-Arabic transcription with Egyptian context, silence filtering, and narrow corrections for confirmed command-word confusions, selects the free Egyptian female Edge voice `ar-EG-SalmaNeural`, keeps automatic TTS disabled for ordinary replies, and adds instructions for Egyptian Arabic conversation. For an explicit Arabic or English Telegram voice request, deterministic gateway intent detection (with negation taking precedence) converts the final response to OGG/Opus even when the selected LLM does not call TTS. Explicit TTS tool calls remain available for multiple comparison samples, and no second `send_message` call is required. Telegram tool progress, streaming, and interim assistant chatter are disabled to avoid Flood Control. Vision is pinned to NaraRouter's multimodal `mistral-medium-3-5` instead of inheriting the text-only main model, with direct Gemini 2.5 Flash as the configured fallback. OpenRouter's free multimodal Gemma route is used only when neither of those credentials is available.
