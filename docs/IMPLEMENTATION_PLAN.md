...
Provide an authenticated Open WebUI portal for Hermes Agent with audited tools, durable workspace evidence, current cloud model routing, local GPU fallback, and a deny-by-default Telegram integration.
...
- Windows Ollama is a final LAN-only fallback.

## Deployment Phases

1. Start Docker services and verify persistent mounts.
2. Validate provider credentials independently.
3. Configure the primary and fallback routing chain.
4. Deploy the seven audited Open WebUI tools.
5. Publish the master, nine specialist, and three cloud preset models.
...
- At least one cloud route and the local fallback are available.
- Seven canonical tools pass their direct harness.
- Thirteen published model profiles pass smoke tests.
